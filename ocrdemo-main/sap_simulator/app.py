...
logger = logging.getLogger('sap_simulator')
...
# Validation Service Configuration (via APIM or direct Function App URL)
VALIDATION_SERVICE_URL = os.getenv('VALIDATION_SERVICE_URL', 'http://localhost:7071')
APIM_SUBSCRIPTION_KEY = os.getenv('APIM_SUBSCRIPTION_KEY', '')
VALIDATION_SERVICE_KEY = os.getenv('VALIDATION_SERVICE_KEY', '')  # fallback for local dev
...
def call_validation_service(record_id: str, delivery_quantity: float, blob_name: str) -> dict:
    """
    POST to the Validation API (via APIM in production, direct in local dev).
    """
...
def simulate_sap():
    ...
    try:
        ...
        logger.info("Uploading %s -> %s/raw/ ...", filename, record_id)
        ...
        # Step 4 – call Azure Validation Function API (via APIM)
        logger.info("Calling validation API for record_id=%s, qty=%s", record_id, delivery_quantity)
        validation_response = call_validation_service(record_id, delivery_quantity, blob_name)
        logger.info("Received validation response for record_id=%s", record_id)
        ...
        logger.warning("Validation service unreachable: %s", e)
...
def health_check():
    ...
    try:
        ...
        if APIM_SUBSCRIPTION_KEY:
            ...
            headers['x-functions-key'] = VALIDATION_SERVICE_KEY
        resp = requests.get(f"{VALIDATION_SERVICE_URL}/api/health", headers=headers, timeout=5)
...
if __name__ == '__main__':
    if not AZURE_STORAGE_ACCOUNT_URL and not AZURE_STORAGE_CONNECTION_STRING:
        logger.warning("AZURE_STORAGE_ACCOUNT_URL / AZURE_STORAGE_CONNECTION_STRING not configured")
        logger.warning("  The SAP simulator will not be able to upload files to blob storage")

    logger.info("SAP Simulator starting...")
    logger.info("  Blob Storage: %s", 'configured' if (AZURE_STORAGE_ACCOUNT_URL or AZURE_STORAGE_CONNECTION_STRING) else 'NOT configured')
    logger.info("  Validation Service: %s", VALIDATION_SERVICE_URL)
    logger.info("  Auth: %s", 'APIM subscription key' if APIM_SUBSCRIPTION_KEY else ('Function key' if VALIDATION_SERVICE_KEY else 'None'))
