...
def validate(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        logging.info("[%s] Validate request received", correlation_id)
        ...
        logging.info("[%s] Validating record_id=%s  qty=%s  blob=%s",
                     correlation_id, record_id, delivery_quantity, blob_name)
        ...
        logging.info("[%s] Cargo discharged weight from document: %s", correlation_id, cargo_weight)
        ...
        logging.info("[%s] record_id=%s  status=%s", correlation_id, record_id, result_payload['status'])
        return _json_response(result_payload)

    except Exception as e:
        logging.exception("[%s] Validation error" % locals().get('correlation_id', 'unknown'))
